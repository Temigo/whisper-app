...
class AlgorithmHenri:
    def __init__(self):
        pass

    def run(self, G, G_i, prob, j=10, Offset=3):
        """
        ...
        for i in range(0, j):  # Gathering the seeds from the actual algorithm
            for seed in self.run_once(G, G_i, prob):
                ss.append(seed)
    ...
    def run_once(self, G, G_i, prob):
        ...
        while len(infected.node) < len(G_i.node):
            old = ss
            ss = self.ripple_step(G_i, frontier, infected, uninfected, prob)
    ...
    def ripple_step(self, G_i, frontier, infected, uninfected, prob):
        """
